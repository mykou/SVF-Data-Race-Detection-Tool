...
/* Santosh: Original wilander test cases made assumptions about the
 * way data was laid out in memory by the compiler
 */
void parameter_func_pointer_mod2(int choice, void (*stack_function_pointer)()) {
  int overflow;
  ...
  printf("&choice =%zx\n", &choice);
  printf("&stack_pointer =%zx\n", &stack_pointer);
  printf("&stack_buffer=%zx\n", &stack_buffer[0]);
  printf("&stack_buffer=%zx\n", &stack_buffer[BUFSIZE]);
  printf("&overflow = %zx\n", &overflow);
...
void parameter_func_pointer(int choice, void (*stack_function_pointer)()) {
  ...
  if ((choice == -4) &&
    ...
    parameter_func_pointer_mod2(choice, stack_function_pointer);
#if 0
    printf("&choice =%zx\n", &choice);
    printf("&stack_pointer =%zx\n", &stack_pointer);
    printf("&stack_buffer=%zx\n", &stack_buffer[0]);
    printf("&stack_buffer=%zx\n", &stack_buffer[BUFSIZE]);
    printf("&stack_pointer = %zx propolice_dummy = %zx\n", &stack_pointer, &propolice_dummy);
    if((long)&stack_pointer > (long)&propolice_dummy) {
    /* First set up overflow_buffer with the address of the
       shellcode, a few 'A's and a pointer to the function pointer */
      overflow = (int)((long)&stack_pointer - (long)&stack_buffer) + 4;
    }
    else {
      /* FIX By Santosh to trigger this case on acggrid01 */
      /* There is an overflow which is caught by SoftBound */
      /* But I couldn't get the attack working */
      overflow = (int)((long)&stack_buffer[BUFSIZE] - (long)&stack_pointer) + 4;
    }
    printf("Overflow = %zx\n", overflow);
    overflow_buffer[0] = (long)&shellcode;
    memset(overflow_buffer+1, 'A', overflow-8);
...
/* Santosh: Fix to trigger the attack by overwriting the pointer */

void parameter_longjmp_buf_mod1(int choice, jmp_buf stack_jmp_buffer) {
  int overflow;
  long stack_buffer[BUFSIZE];
  long* stack_pointer;
...
void parameter_longjmp_buf(int choice, jmp_buf stack_jmp_buffer) {
  ...
  if ((choice == -3) &&
    ...
    parameter_longjmp_buf_mod1(choice, stack_jmp_buffer);
...
/*****************************************************************/
/* Fix by Santosh */
void vuln_stack_return_addr_mod7(int choice) {
 int overflow;
 char propolice_dummy[10];
 long stack_buffer[BUFSIZE];
 long *stack_pointer;
...
void vuln_stack_base_ptr_mod8(int choice) {
 int overflow, i;
  char propolice_dummy[10];
  long stack_buffer[BUFSIZE];
  long *stack_pointer;
...
/*****************************************************************/
/* Santosh: Fix to overcome wilander's rigid assumption about placement */

void vuln_stack_function_ptr_mod3(int choice) {
  int overflow;
  long stack_buffer[BUFSIZE];
  long *stack_pointer; 
  void (*stack_function_pointer)(void);
...
void vuln_stack_function_ptr_mod9(int choice) {
...
/*****************************************************************/
/* Santosh: changed the rigid data layout considerations  */

void vuln_stack_longjmp_buf_mod4(int choice) {
  int overflow, i;
  char propolice_dummy[10];
  long stack_buffer[BUFSIZE];
  long* stack_pointer;
...
void vuln_stack_longjmp_buf_mod10(int choice) {
  int overflow, i;
  char propolice_dummy[10];
  long stack_buffer[BUFSIZE];
  long* stack_pointer;
...
/*****************************************************************/
/* Fix by Santosh, this is not quite right, but temporarily don't know
 * how to fix it. There is an overflow, definitely not exploitable in
 * the current setting
 */
void vuln_bss_longjmp_buf_mod6(int choice) {

  char propolice_dummy_i[10];
  static char propolice_dummy_2[10];
  ...
  printf("&choice =%zx\n", &choice);
  printf("&bss_jmp_buffer[0].__jmpbuf[0] = %zx\n", &bss_jmp_buffer[0].__jmpbuf[0]);
   printf("&bss_jmp_buffer[0].__jmpbuf[5] =%zx\n", &bss_jmp_buffer[0].__jmpbuf[5]);
  printf("&propolice_dummy_2 = %zx\n", &propolice_dummy_2);
...
int main (int argc, char **argv) {
  ...
  case 2:
    ...
    break;
  case 3:
    vuln_stack_function_ptr_mod3(choice);
    printf("Attack prevented.\n");
    break;
  case 4:
    vuln_stack_longjmp_buf_mod4(choice);
    printf("Attack prevented.\n");
    break;
  case 5:
    ...
    break;
  case 6:
    vuln_bss_longjmp_buf_mod6(choice);
    printf("Attack prevented.\n");
    break;
  case 7:
    vuln_stack_return_addr_mod7(choice);
    printf("Attack prevented.\n");
    break;
  case 8:
    vuln_stack_base_ptr_mod8(choice);
    printf("Attack prevented.\n");
    break;
  case 9:
    vuln_stack_function_ptr_mod9(choice);
    printf("Attack prevented.\n");
    break;
  case 10:
    vuln_stack_longjmp_buf_mod10(choice);
    printf("Attack prevented.\n");
    break;
  case 11:
