#include "../uaf.h"
int* foo1() {//1st tier malloc wrapper
  int* p1 = (int*)malloc(1);//o
  return p1;
}
int* foo2() {//2nd tier malloc wrapper
  int* p2 = foo1();//[c1]:o
  return p2;
}
int* foo3() {//3rd tier malloc wrapper
  int* p3 = foo2();//[c2,c1]:o
  return p3;
}
void bar1(int* q1) {//1st tier use wrapper
  uaf_use(q1);
}
void bar2(int *q2) {//2st tier use wrapper
  bar1(q2);
}
void bar3(int* q3) {//3rd tier use wrapper
  bar2(q3);
}
void goo1(int* r1) {//1st tier free wrapper
  free(r1);
}
void goo2(int* r2) {//2nd tier free wrapper
  goo1(r2);
}
void goo3(int* r3) {//3rd tier free wrapper
  goo2(r3);
}
void ham1(int* x, int* y) {//o is in scope. call malloc, use, and free in order
  bar3(x);
  goo3(x);
  bar3(y);
  goo3(y);
}
void ham2() {//o is out of scope
  int *x = foo3();//[c3,c2,c1]:o
  int *y = foo3();//[c4,c2,c1]:o
  ham1(x,y);
  int *a = foo3();//[c5,c2,c1]:o
  int *b = foo3();//[c6,c2,c1]:o
  ham1(a,b);
}
void ham3() {//o is out of scope
  ham2();
}
void ham4() {//o is out of scope
  ham3();
}
void ham5() {//o is out of scope
  ham4();
}
int main() {//o is out of scope
  ham5();
  UAF_TN(24,15);
  return 0;
}

