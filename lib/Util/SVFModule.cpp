...
SVFModule *SVFModule::svfModule = NULL;

SVFModule::SVFModule(const vector<string> &moduleNameVec) {
...
void SVFModule::build(const vector<string> &moduleNameVec) {
    loadModules(moduleNameVec);
    initialize();
    buildFunToFunMap();
    buildGlobalDefToRepMap();
...
void SVFModule::loadModules(const std::vector<std::string> &moduleNameVec) {
...
void SVFModule::buildFunToFunMap() {
...
void SVFModule::buildGlobalDefToRepMap() {
...
void SVFModule::dumpModulesToFile(const std::string suffix) {
    for (u32_t i = 0; i < moduleNum; ++i) {
