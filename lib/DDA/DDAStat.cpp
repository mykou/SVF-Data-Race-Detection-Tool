...
DDAStat::DDAStat(FlowDDA* pta) : PTAStat(pta), flowDDA(pta), contextDDA(NULL), pathDDA(NULL) {
    initDefault();
}
DDAStat::DDAStat(ContextDDA* pta) : PTAStat(pta), flowDDA(NULL), contextDDA(pta), pathDDA(NULL) {
    initDefault();
}
DDAStat::DDAStat(PathDDA* pta) :	PTAStat(pta), flowDDA(NULL), contextDDA(NULL), pathDDA(pta) {
    initDefault();
}
...
void DDAStat::performStatPerQuery(NodeID ptr) {
    ...
    if(flowDDA) {
        ...
        for(PathDDA::LocToDPMVecMap::const_iterator it =  pathDDA->getLocToDPMVecMap().begin(),
                eit = pathDDA->getLocToDPMVecMap().end(); it!=eit; ++it) {
            NumOfLoc++;
            u32_t num = it->second.size();
            NumOfDPM += num;
            if(num > maxNumOfDPMPerLoc)
        ...
        VFPathCond path;
        VFPathVar var(path,ptr);
        cptsSize = pathDDA->getPts(var).count();
        pts = pathDDA->getBVPointsTo(pathDDA->getPts(var));
    }
...
void DDAStat::getNumOfOOBQuery()
{
    if (flowDDA)
        ...
        _TotalNumOfOutOfBudgetQuery = pathDDA->outOfBudgetDpms.size();
}
...
void DDAStat::printStat() {
    ...
    if(flowDDA) {
        ...
        pathDDA->getSVFG()->getStat()->performSCCStat(pathDDA->getInsensitiveEdgeSet());
    }
