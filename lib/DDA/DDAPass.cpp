...
static cl::bits<PointerAnalysis::PTATY> DDASelected(cl::desc("Select pointer analysis"),
        cl::values(
            ...
            clEnumValN(PointerAnalysis::PathS_DDA, "dps", "Guarded value-flow Demand-driven analysis")
            ));
...
void DDAPass::selectClient(llvm::Module& module) {
    ...
    if (!userInputQuery.empty()) {
        ...
        else if (userInputQuery == "cast") {
            _client = new CastDDAClient(module);
        }
        /// check use after free
        else if (userInputQuery == "free") {
            _client = new FreeDDAClient(module);
        }
        /// check uninitialised variables
        else if (userInputQuery == "uninit") {
            _client = new UninitDDAClient(module);
        }
...
void DDAPass::runPointerAnalysis(llvm::Module& module, u32_t kind)
{
    ...
    switch (kind) {
    case PointerAnalysis::PathS_DDA: {
        _pta = new PathDDA(&module, _client);
        break;
    }
