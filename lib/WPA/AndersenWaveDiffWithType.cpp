...
void AndersenWaveDiffWithType::processTypeMismatchedGep(NodeID obj, const Type *type) {
    ...
    std::set<const NormalGepCGEdge*> processed;

    PTAType ptaTy(type);
    NodeBS &nodesOfType = typeSystem->getVarsForType(ptaTy);

    for (std::set<const NormalGepCGEdge*>::iterator nit = edges.begin(), neit = edges.end(); nit != neit; ++nit) {
        const NormalGepCGEdge *normalGepEdge = *nit;
    ...
    for (std::set<const NormalGepCGEdge*>::iterator nit = processed.begin(), neit = processed.end(); nit != neit; ++nit)
        edges.erase(*nit);
}
