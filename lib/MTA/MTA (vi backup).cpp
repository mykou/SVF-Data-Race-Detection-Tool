...
bool isShared(const llvm::Value *val, llvm::Module& module){
    ...
    for (PointsTo::iterator it = target.begin(), eit = target.end();
     ...
     // points to
    /*
    const MemObj *obj = pag->getObject(pag->getObjectNode(val));
    std::cout << pag << " " << val << " " << pag->getObjectNode(val)  << std::endl;
    if (obj == NULL) return false;
    std::cout << "get here?\n";
    if (obj->isGlobalObj()){
        std::cout << "is global\n";
    }
    return (obj->isGlobalObj() || obj->isStaticObj() || obj->isHeap());*/
    return false;
}
...
void MTA::pairAnalysis(llvm::Module& module, MHP *mhp, LockAnalysis *lsa){
    ...
    int count = 0;
    std::vector<const Instruction *> cache;
    ...
    for (auto it = instructions.cbegin(); it != instructions.cend(); it++){   
        ...
        auto tmp = it;
        for (auto it2 = ++tmp; it2 != instructions.cend();){   
             std::cout<<"hello1"<<std::endl;
             std::cout<<++count2<<std::endl;
            InstructionPair pair = InstructionPair(*it,*it2);
                                    std::cout<<"hello2"<<std::endl;
            //std::cout << "line = " << getSourceLoc(*it2) <<std::endl;
            if (hasDataRace(pair, module,mhp,lsa)){
                                                                std::cout<<"hello2.5"<<std::endl;

                pairs.push_back(pair);
                                                                                std::cout<<"hello2.6"<<std::endl;

                cache.push_back(*it2);
                                                                                std::cout<<"hello2.7"<<std::endl;

                //first iterator right after going into the second loop, need to increment otherwise
                //it will get invalidated

                                std::cout<<"hello3"<<std::endl;
                                                                    
                if(it2 == instructions.cend()) {
        ...
        //put all the cache items and pair them up
        for(auto it3 = cache.begin(); it3 != cache.end(); ++it3) {
            for(auto it4 = ++it3; it4 != cache.end(); ++it4) {
                InstructionPair pair = InstructionPair(*it3, *it4);
                pairs.push_back(pair);
            }
        }
                                                        std::cout<<"hello7"<<std::endl;

    //do the rest of the analysis here.
        int count1 = 0;
        for(auto it3 = pairIt; it3 != pairs.end(); ++it3) {
            //if true, keep it
            //increment
            //else remove
    ...
    // std::vector<InstructionPair> cache;
    // for (auto it = instructions.cbegin(); it != instructions.cend();++it){
    //     for (auto it2 = ++it; it2 != instructions.cend();){
    //         ++count;
