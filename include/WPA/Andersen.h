...
private:
    ...
    typedef std::map<NodeID, std::set<const NormalGepCGEdge*>> TypeMismatchedObjToEdgeTy;

    TypeMismatchedObjToEdgeTy typeMismatchedObjToEdges;

    void recordTypeMismatchedGep(NodeID obj, const NormalGepCGEdge* normalGepEdge) {
        TypeMismatchedObjToEdgeTy::iterator it = typeMismatchedObjToEdges.find(obj);
        if (it != typeMismatchedObjToEdges.end()) {
            std::set<const NormalGepCGEdge*> &edges = it->second;
            edges.insert(normalGepEdge);
        } else {
            std::set<const NormalGepCGEdge*> edges;
            edges.insert(normalGepEdge);
...
public:
    ...
    /// Create an singleton instance directly instead of invoking llvm pass manager
    static AndersenWaveDiffWithType* createAndersenWaveDiffWithType(llvm::Module& module) {
        if(diffWaveWithType==NULL) {
            diffWaveWithType = new AndersenWaveDiffWithType();
            diffWaveWithType->analyze(module);
